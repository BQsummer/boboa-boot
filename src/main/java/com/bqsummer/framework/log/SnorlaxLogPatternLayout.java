...
public class SnorlaxLogPatternLayout extends LayoutBase<ILoggingEvent> {
    ...
    public SnorlaxLogPatternLayout() {
    }
    ...
    private void appendTraceInfo(Converter<ILoggingEvent> c, StringBuilder strBuilder, ILoggingEvent event) {
        LogType logType = LogType.valueOf(MDC.get("logType"));
        String logMessage = c.convert(event);

        try {
            String target = "[" + logType + "]: ";
            String json = logMessage.substring(logMessage.indexOf(target) + target.length());
            strBuilder.append(target);
            strBuilder.append(json.substring(0, json.lastIndexOf("}")));
            strBuilder.append(",\"traceId\":\"").append(StringUtils.isEmpty(MDC.get("traceId")) ? "noTraceId" : MDC.get("traceId")).append("\"");
            strBuilder.append("}");
        } catch (Exception var8) {
            strBuilder.append("invalid " + logType + ": " + logMessage);
        }
    }
